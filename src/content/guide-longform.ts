...
function buildGuideSupportParagraph(
  ...
  if (language === 'ko') {
    if (family === 'saju') {
      return `${sectionHeading}을 읽을 때는 일간, 계절성, 주변 오행의 압력처럼 기본 축을 함께 떠올리는 편이 좋습니다. 그래야 기초 설명이 관계 해석이나 직업 해석으로 넘어갈 때도 끊기지 않고 이어지며, 계산 결과를 다시 볼 때 어떤 항목부터 확인해야 하는지도 자연스럽게 정리됩니다.`
    }

    if (family === 'ziwei') {
      return `${sectionHeading} 구간은 궁과 별을 따로 외우는 대신, 둘이 만났을 때 어떤 장면이 만들어지는지 상상해보면 훨씬 잘 남습니다. 명반 해석은 세부 용어를 많이 아는 것보다 질문의 초점을 분리해 읽는 습관이 더 중요하기 때문에, 이 단계에서 읽기 순서를 잡아두는 것이 특히 유용합니다.`
    }

    return `${sectionHeading}을 이해할 때는 행성의 기능, 하우스의 무대, 각도의 긴장과 조화를 같이 떠올리는 편이 좋습니다. 차트를 잘 읽는 사람일수록 하나의 상징을 과장하기보다 구조 안에서 역할을 비교하므로, 이 문단도 같은 방식으로 따라가면 훨씬 실제적인 이해로 이어집니다.`
  }
