...
export default function AspectGrid({ aspects }: Props) {
  ...
  return (
    <div>
      <h3 className="text-sm font-medium text-base-content/70 mb-2">{t.natal.aspects}</h3>
      <div className="space-y-0.5">
        {top.map((a, i) => (
          <div key={i} className="flex items-center gap-2 text-base py-0.5">
            ...
            <span className="text-base-content/70 w-12">{PLANET_KO[a.planet1]}</span>
            <span className="w-4 text-center">{ASPECT_SYMBOLS[a.type]}</span>
            <span className="w-5 text-center">{PLANET_SYMBOLS[a.planet2]}</span>
            <span className="text-base-content/70 w-12">{PLANET_KO[a.planet2]}</span>
            <span className="ml-auto font-mono text-base-content/60 text-sm">
              orb {a.orb.toFixed(1)}°
            </span>
          </div>
        ))}
